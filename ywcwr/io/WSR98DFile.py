...
class WSR98DBaseData(object):
    ...
    def __init__(self, filename, station_lon=None, station_lat=None, station_alt=None):
        ...
        self.product_type = [list(self.radial[i]['fields'].keys()) for i in self.sweep_start_ray_index]
...
class WSR98D2NRadar(object):
    ...
    def __init__(self, WSR98D):
        self.WSR98D = WSR98D
        self.product_types = self.WSR98D.product_type
        ...
        self.radial = [iray for ind, iray in enumerate(self.WSR98D.radial)]
        ...
        self.latitude, self.longitude, self.altitude, self.frequency = self.WSR98D.get_latitude_longitude_altitude_frequency()
        ...
        self.range_v = self.get_range_per_radial(self.max_bins, type='doppler')
        self.range_z = self.get_range_per_radial(self.max_bins, type='dbz')
    ...
    def get_reomve_radial_num(self):
        ...
        for isweep in dBZ_alone:
            index_romove.extend(range(self.WSR98D.sweep_start_ray_index[isweep], self.WSR98D.sweep_end_ray_index[isweep] + 1))
        return index_romove
    ...
    def get_nbins_per_sweep(self):
        ...
        return np.array([self.radial[idx]['fields'][self.product_types[id][0]].size for id, idx in
                         enumerate(self.sweep_start_ray_index)])

    def get_range_per_radial(self, length, type="doppler"):
        """
        确定径向每个库的距离 range变量
        :param length:
        :return:
    ...
    def _get_fields(self):
        ...
        field_keys = list(set([i for j in self.product_types for i in j]))
    ...
    def ToPRD(self):
        """将WSR98D数据转为PRD的数据格式"""
        sitecode = None
        if self.sitename.isalpha():
            sitecode = 9720
        elif 'ChangShaBX' in self.sitename:
            sitecode = self.sitename
        else:
            # sitecode = self.sitename.split('_')[1]
            sitecode = self.sitename
        return PRD(fields=self.fields, scan_type=self.scan_type, time=self.get_scan_time(),
                          rangev=self.range_v, rangez=self.range_z, azimuth=self.azimuth, elevation=self.elevation, latitude=self.latitude,
                          ...
                          nrays=self.nrays, nsweeps=self.nsweeps, sitename=self.sitename, sitecode=sitecode, products=self.product_types, pyart_radar=self.ToPyartRadar())
