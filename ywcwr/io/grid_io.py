...
def write_grid(filename, grid, format='NETCDF4',
               ...
               write_point_x_y_z=False, write_point_lon_lat_alt=False, write_lon_lat=False):
    """
    ...
    dset.createDimension('latitude', grid.ny)
    dset.createDimension('longitude', grid.nx)


    # dset.createDimension('lat', grid.point_latitude)
    if grid.nradar != 0:
    ...
    var_lat = dset.createVariable("latitude", np.float32, dimensions=("latitude",))
    var_lon = dset.createVariable("longitude", np.float32, dimensions=("longitude",))
    ...
    # _create_ncvar(lat[:, 0], dset, 'latitude', ('latitude',))
    # _create_ncvar(lon[0, :], dset, 'longitude', ('longitude',))
    ...
    for field, field_dic in grid.fields.items():
        ...
        field_dic['grid_mapping'] = projparams['proj']
        _create_ncvar(field_dic, dset, field, ('time', 'z', 'latitude', 'longitude'))
    ...
    if 'Conventions' not in dset.ncattrs():
        dset.setncattr('Conventions', 'GRID-1.1')
