...
class Graph(object):
    ...
    def plot_ppi(self, ax, sweep_num, field_name, cmap=None, min_max=None, cmap_bins=None, cbar=True,
        ...
        gci = ax.pcolormesh(x / 1000., y / 1000., radar_data, cmap=cmaps, zorder=0, norm=norm, shading='auto', **kwargs)
        if cbar:
...
class GraphMap(object):
    ...
    def plot_ppi_map(self, ax, sweep_num, field_name, extend=None, cmap=None, min_max=None,\
                 cmap_bins=None, cbar=True, orientation="vertical",cbar_ticks=None, cbar_ticklabels=None, \
                     clabel=None, levels=None, **kwargs):
        """
        ...
        if levels is None:
            levels = MaxNLocator(nbins=cmap_bins).tick_values(vmin, vmax)
        # print(levels, vmax,vmin)
        norm = BoundaryNorm(levels, ncolors=cmaps.N, clip=True)
        # clevs = [-50, -38, -33, -31, -28, -26, -23, -21, -18, -16, -14, -11, -8, -3, 2, 10, 18, 24, 32, 50]
        ...
        # parallels = np.arange(int(min_lat), np.ceil(max_lat) + 0.01, 0.1)
        # meridians = np.arange(int(min_lon), np.ceil(max_lon) + 0.01, 0.1)
        parallels = np.arange(int(min_lat), np.ceil(max_lat) + 0.5, 0.5)
        meridians = np.arange(int(min_lon), np.ceil(max_lon) + 0.5, 0.5)
