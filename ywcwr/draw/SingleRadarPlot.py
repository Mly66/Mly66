...
class RadarGraph(object):
    ...
    def simple_plot_ppi_xy(x, y, radar_data, normvar=None, cmap=None,
        ...
        plt.rcParams['axes.unicode_minus'] = False  # 设置字体之后不支持负号
        fig = plt.figure()
    ...
    def plot_ppi(fig, ax, cx, x, y, radar_data, max_range=None, title=None, normvar=None, cmap=None, \
        ...
        """
        if label is None:
            xlabel = "由西向东方向到雷达的距离（单位：km）"
            ylabel = "由南向北方向到雷达的距离（单位：km）"
        else:
            xlabel, ylable = label
        if normvar is None:
            vmin = -5
        ...
        gci = ax.pcolormesh(x / 1000., y / 1000., radar_data, cmap=cmaps, \
                            zorder=0, norm=norm)
        max_range = range_cycle / 1000.
        deltaR = max_range // 5
        RadarGraph._SetGrids(ax, max_range, deltaR=deltaR)
        ax.set_aspect("equal")
        ...
        if max_range > 50:
            ...
            major, minor = 10, 1
        RadarGraph._SetAxis(ax, major, minor, 8)
