...
from .RadarGrid import get_CR_xy, get_CAPPI_xy
...
class PRD(object):
    ...
    def __init__(self, fields,  scan_type, time, rangev, rangez, azimuth, elevation,latitude,
                 ...
                 nrays, nsweeps, sitename, sitecode, products, pyart_radar=None, iPolarzation='水平', field_name='dBZ'):
        super(PRD, self).__init__()
        keys = fields.keys()
        self.fields = []
        for idx, (istart, iend) in enumerate(zip(sweep_start_ray_index, sweep_end_ray_index)):
            if 'dBZ' in products[idx]:
                range = rangez
            else:
                range = rangev
            ...
            # for ikey in keys:
            #     if ikey not in products[idx]:
            #         isweep_data = isweep_data.drop_vars(ikey)
            self.fields.append(isweep_data)
        if nrays % 360 != 0 and len(products) < 2:
            self.scan_info = xr.Dataset(data_vars={"latitude": latitude, "longitude": longitude,
                                               "altitude": altitude, 'site_id': sitecode, "scan_type": scan_type, "frequency": frequency,
                                               "start_time": time[0], "end_time": time[-1],
                                               "nyquist_velocity": (['sweep', ], nyquist_velocity[:nsweeps]),
                                               "unambiguous_range": (['sweep', ], [1]),
                                               "polarzation_type": iPolarzation,
                                               "rays_per_sweep": (
                                               ['sweep', ], sweep_end_ray_index - sweep_start_ray_index + 1),
                                               "fixed_angle": (["sweep", ], [1]),
                                               "beam_width": (
                                               ["sweep", ], nrays / (sweep_end_ray_index - sweep_start_ray_index + 1))},
            ...
            self.scan_info = xr.Dataset(data_vars={"latitude": latitude, "longitude": longitude,
                                                   ...
                                                   "beam_width": (
                                                       ["sweep", ],
                                                       360. / (sweep_end_ray_index - sweep_start_ray_index + 1))},
                                        coords={"sweep": np.arange(nsweeps, dtype=int)})
        ...
        self.get_vol_data(field_name=field_name)
    ...
    def add_product_CAPPI_lonlat(self, XLon, YLat, level_height):
        ...
        proj = pyproj.Proj(projparams)
        GridX, GridY = proj(GridLon, GridLat, inverse=False)
        vol_azimuth, vol_range, fix_elevation, vol_value, radar_height, radar_lon_0, radar_lat_0 = self.vol
        vol_azimuth = [vol_azimuth[i] for i in range(self.nsweeps) if 'dBZ' in self.fields[i]]
        vol_range = [vol_range[i] for i in range(self.nsweeps) if 'dBZ' in self.fields[i]]
        fix_elevation = np.array([fix_elevation[i] for i in range(self.nsweeps) if 'dBZ' in self.fields[i]])
        vol_value = [vol_value[i] for i in range(self.nsweeps) if 'dBZ' in self.fields[i]]
    ...
    def get_vol_data(self, field_name="dBZ", fillvalue=-999.):
        ...
        vol_value = []
        for ppi in order_vol.fields:
            try:
                ppi_value = np.where(np.isnan(ppi[field_name].values), fillvalue, ppi[field_name].values).astype(np.float64)
            except:
                ppi_value = np.where(np.isnan(ppi['V'].values), fillvalue, ppi['V'].values).astype(np.float64)
            vol_value.append(ppi_value)
