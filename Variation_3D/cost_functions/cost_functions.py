...
def J_function(winds, parameters):
    ...
    if parameters.engine == "tensorflow":
        if not TENSORFLOW_AVAILABLE:
            raise ImportError("Tensorflow 2.5 or greater is needed in order to use TensorFlow-based Variation_3D!")
        ...
        if not JAX_AVAILABLE:
            raise ImportError("Jax is needed in order to use the Jax-based Variation_3D!")
