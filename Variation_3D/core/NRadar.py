...
class PRD(object):
    ...
    def __init__(self, fields,  scan_type, time, range, azimuth, elevation, latitude,
        ...
        self.scan_info = xr.Dataset(data_vars={"latitude": latitude, "longitude": longitude,
                                               ...
                                               "beam_width": (
                                               ["sweep", ], 360. / (sweep_end_ray_index - sweep_start_ray_index + 1))},
        ...
        #     for idx, (istart, iend) in enumerate(zip(sweep_start_ray_index, sweep_end_ray_index)):
        #         if sweep_layer == idx:
        #             x, y, z = antenna_vectors_to_cartesian_cwr(range[:bins_per_sweep[0]], azimuth[:], \
        #                                                        elevation[:], altitude)
        #             lon, lat = cartesian_to_geographic_aeqd(x, y, longitude, latitude)
        #             isweep_data = xr.Dataset(coords={'azimuth': (['time', ], azimuth[:]),
        ...
        #                                              'lat': (['time', 'range'], lat),
        #                                              'lon': (['time', 'range'], lon),
        #                                              'range': range[:bins_per_sweep[0]], 'time': time[istart:iend + 1]})
        #             isweep_data.azimuth.attrs = DEFAULT_METADATA['azimuth']
        #             isweep_data.elevation.attrs = DEFAULT_METADATA['elevation']
        #             isweep_data.range.attrs = DEFAULT_METADATA['range']
        #             isweep_data.time.attrs = DEFAULT_METADATA['time']
        ...
        #             # if sweep_layer == idx:
        #             for ikey in keys:
        #                 isweep_data[ikey] = (['time', 'range'], fields[ikey][:, :bins_per_sweep[0]])
        #                 isweep_data[ikey].attrs = DEFAULT_METADATA[CINRAD_field_mapping[ikey]]
        #             self.fields.append(isweep_data)
        #         else:
        #             continue
            ...
            #                                        "altitude": altitude, "scan_type": scan_type, "frequency": frequency,
            #                                        "start_time": time[0], "end_time": time[-1],
            #                                        "nyquist_velocity": (["sweep", ], nyquist_velocity[sweep_layer:sweep_layer+1]),
            #                                        "unambiguous_range": (["sweep", ], unambiguous_range[sweep_layer:sweep_layer+1]),
            #                                        "polarzation_type": iPolarzation,
            #                                        "rays_per_sweep": (
            #                                            ["sweep", ], (sweep_end_ray_index - sweep_start_ray_index + 1)[sweep_layer:sweep_layer+1]),
            #                                        "fixed_angle": (["sweep", ], fixed_angle[sweep_layer:sweep_layer+1]),
            #                                        "beam_width": (
            #                                            ["sweep", ],
            #                                            360. / (sweep_end_ray_index - sweep_start_ray_index + 1)[sweep_layer:sweep_layer+1])},
            #                             coords={"sweep": np.array([sweep_layer], dtype=int)})
            # self.scan_info['latitude'].attrs = DEFAULT_METADATA['latitude']
            # self.scan_info['longitude'].attrs = DEFAULT_METADATA['longitude']
            # self.scan_info['altitude'].attrs = DEFAULT_METADATA['altitude']
            # self.scan_info['scan_type'].attrs = DEFAULT_METADATA['scan_type']
            ...
            # self.nrays = nrays
            # self.sitename = sitename
            # self.get_vol_data(field_name=field_name, sweep_layer=sweep_layer)
            # self.product = xr.Dataset()
            # self.PyartRadar = pyart_radar
    ...
    def ordered_az(self, inplace=False):
        ...
        # else:
        #     if inplace:
        #         self.fields[0] = self.fields[0].swap_dims({"time": "azimuth"}).sortby("azimuth") ##对数据重新排序
        #         return None
        #     else:
        #         prd_dat = PRD_AZ()
        #         prd_dat.scan_info = self.scan_info
        #         prd_dat.fields.append(self.fields[0].swap_dims({"time": "azimuth"}).sortby("azimuth"))
        #         return prd_dat
