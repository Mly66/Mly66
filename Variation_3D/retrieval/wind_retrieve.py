...
class DDParameters(object):
    """
    This is a helper class for inserting more arguments into the :func:`Variation_3D.cost_functions.J_function` and
    :func:`Variation_3D.cost_functions.grad_J` function. Since these cost functions take numerous parameters, this class
...
def _get_dd_wind_field_scipy(Grids, u_init, v_init, w_init, engine,
                             ...
                             max_iterations=50, mask_w_outside_opt=True,
...
def _get_dd_wind_field_tensorflow(Grids, u_init, v_init, w_init, points=None, vel_name=None,
                                  ...
                                  max_bca=150.0, upper_bc=True, model_fields=None,
                                  output_cost_functions=True, roi=1000.0,
                                  parallel_iterations=4, wind_tol=0.1):
    if not TENSORFLOW_AVAILABLE:
    ...
    for i in range(10):
        a = parameters.azs[0][i]
        b = parameters.els[0][i]
    ...
    parameters.vrs = [tf.constant(x.filled(-9999), dtype=tf.float32) for x in parameters.vrs]
    parameters.azs = [tf.constant(x.filled(-9999), dtype=tf.float32) for x in parameters.azs]
    parameters.els = [tf.constant(x.filled(-9999), dtype=tf.float32) for x in parameters.els]
    parameters.wts = [tf.constant(x.filled(-9999), dtype=tf.float32) for x in parameters.wts]
    ...
    sum_Vr = tf.experimental.numpy.nansum(tf.square(parameters.vrs * parameters.weights))
    parameters.rmsVr = np.sqrt(np.nansum(sum_Vr) / tf.experimental.numpy.nansum(parameters.weights))
    ...
    while (iterations < max_iterations and (abs(wprevmax - wcurrmax) > 0.05)):
        wprevmax = wcurrmax
        parameters.print_out = False
        ...
        winds = np.reshape(winds.position.numpy(), (3, parameters.grid_shape[0], parameters.grid_shape[1], parameters.grid_shape[2]))
...
def get_dd_wind_field(Grids, u_init, v_init, w_init, engine="scipy", **kwargs):
    ...
    and HRRR data in :py:func:`Variation_3D.constraints`:
    ...
    engine: str (one of "scipy", "tensorflow", "jax")
        ...
        Using Tensorflow or Jax expands Variation_3D's capabiability to take advantage of GPU-based systems.
    ...
    points: None or list of dicts
        Point observations as returned by :func:`Variation_3D.constraints.get_iem_obs`. Set
        to None to disable.
    vel_name: string
        Name of radial velocity field. Setting to None will have Variation_3D attempt
        to automatically detect the velocity field name.
    refl_field: string
        Name of reflectivity field. Setting to None will have Variation_3D attempt
    ...
    Vt: float
        ...
        This is only needed if Cv is not zero.
    filter_winds: bool
        If this is True, Variation_3D will run a low pass filter on
    ...
    model_fields: list of strings
        ...
        in ::Variation_3D.constraints::. Variation_3D will look for fields named U_(model
