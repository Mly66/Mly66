...
def test_add_era_interim_field():
    ...
    Grid0 = 3D.constraints.make_constraint_from_era_interim(
        Grid0, 3D.tests.sample_files.ERA_PATH,
        vel_field='corrected_velocity')
    grid_time = datetime.strptime(Grid0.time["units"],
                                  "seconds since %Y-%m-%dT%H:%M:%SZ")
    era_dataset = Dataset(3D.tests.sample_files.ERA_PATH)
...
def test_era_initialization():
    ...
    Grid0 = 3D.constraints.make_constraint_from_era_interim(
        Grid0, 3D.tests.sample_files.ERA_PATH,
        vel_field='corrected_velocity')
    u_init, v_init, w_init = 3D.initialization.make_initialization_from_era_interim(
        Grid0, 3D.tests.sample_files.ERA_PATH,
        vel_field='corrected_velocity')
