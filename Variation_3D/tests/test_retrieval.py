...
def test_make_updraft_from_convergence_field():
    ...
    x_center = 0.0
    y_center = 0.0
    u, v, w = 3D.tests.make_test_divergence_field(
        Grid, wind_vel, z_ground, z_top, radius, back_u, back_v,
        x_center, y_center)

    new_grids = 3D.retrieval.get_dd_wind_field([Grid], u, v, w, Co=0.0,
...
def test_twpice_case_jax():
    ...
    Grid0 = pyart.io.read_grid(3D.tests.EXAMPLE_RADAR0)
    Grid1 = pyart.io.read_grid(3D.tests.EXAMPLE_RADAR1)
    sounding = pyart.io.read_arm_sonde(3D.tests.SOUNDING_PATH)

    u_init, v_init, w_init = 3D.initialization.make_wind_field_from_profile(
        Grid1, sounding[1], vel_field='corrected_velocity')

    Grids = 3D.retrieval.get_dd_wind_field(
...
def test_smoothing():
    ...
    w = np.zeros((20, 40, 40))
    new_grids = 3D.retrieval.get_dd_wind_field(
