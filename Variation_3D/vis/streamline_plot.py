...
def plot_horiz_xsection_streamlines(Grids, ax=None,
                                    ...
                                    contour_alpha=0.7, arrowsize=1.0,
                                    linewidth=None):
    """
    This procedure plots a horizontal cross section of winds from wind fields
    generated by 3D using streamlines. The density of streamlines varies 
    ...
    title_flag: bool
        If True, 3D will generate a title for the plot.
    axes_labels_flag: bool
        If True, 3D will generate axes labels for the plot.
    colorbar_flag: bool
        If True, 3D will generate a colorbar for the plot background field.
    colorbar_contour_flag: bool
        If True, 3D will generate a colorbar for the contours.
    bg_grid_no: int
    ...
    ax.streamplot(grid_x[level, :, :], grid_y[level, :, :], u[level, :, :],
                  v[level, :, :], color='k', linewidth=linewidth,
                  arrowsize=arrowsize)
    ...
    if(title_flag is True):
        ax.set_title(('3D retreived winds @' + str(grid_h[level, 0, 0]) +
                      ' km'))
...
def plot_horiz_xsection_streamlines_map(Grids, ax=None,
    ...
    ax.streamplot(grid_lon, grid_lat, u[level], v[level],
                  transform=transform, zorder=1, color='k',
    ...
    if(title_flag is True):
        ax.set_title(
            ('3D retreived winds @' + str(grid_h[level, 0, 0]) + ' km'))
...
def plot_xz_xsection_streamlines(Grids, ax=None,
    ...
    generated by 3D in the X-Z plane using streamlines.
    ...
    ax.streamplot(grid_x[:, level, :], grid_h[:, level, :], u[:, level, :],
                  w[:, level, :], color='k', arrowsize=arrowsize,
                  linewidth=linewidth)
    ...
    if(title_flag is True):
        if(grid_y[0, level, 0] > 0):
            ax.set_title(('3D retreived winds @' +
...
def plot_yz_xsection_streamlines(Grids, ax=None,
    ...
    """
    This procedure plots a cross section of winds from wind fields
    generated by 3D in the Y-Z plane using streamlines.
    The density of streamlines varies with horizontal wind speed.
